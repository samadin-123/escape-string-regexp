...
Improve the performance of the `escapeStringRegexp()` function, measured by operations per second when escaping strings containing RegExp special characters. The function currently uses two sequential `.replace()` calls with regex patterns.

## What you CAN modify

- `index.js` — main source code (the escapeStringRegexp function)
